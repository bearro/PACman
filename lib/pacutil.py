...
def sign_vote(votestr, mnprivkey):
    ...
    for i in range(4):
        ...
        if verify_pac_signature(generator_secp256k1, address, msghash, sig):
            return sig


def verify_pac_signature(generator, address, message, signature):
    compressed = False
    G = generator
    curve = G.curve()
    order = G.order()
    ...
    key = Key(public_pair=(Q.x(), Q.y()), netcode='PAC')
