...
git_dir = os.path.abspath(
    os.path.join(
        ...
        '..'))
pac_conf_dir = os.path.join(os.getenv('HOME'), '.paccore')
pac_cli_path = os.getenv('PAC_CLI')
if os.getenv('PACMAN_PID') is None:
    quit("--> please run using 'pacman vote'")
...
def run_pac_cli_command(cmd):
    return run_command("%s %s" % (pac_cli_path or 'pac-cli', cmd))
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        ...
        for mn in shuffled_mn_keys:
            ...
            if background_send:
                deferred_votes = tempfile.NamedTemporaryFile(prefix=('sending_pacvote_votes-%s=' % batch_timestamp),delete=False)
            ...
            for vote in shuffled_vote_keys:
                ...
                signature = pacutil.sign_vote(netvote, mnprivkey)
                command = ('%s' % pac_cli_path is not None and pac_cli_path or 'pac-cli') + ' voteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(
        ...
        if background_send:
            voter_parent = tempfile.NamedTemporaryFile(prefix=('sending_pac_votes-%s-' % batch_timestamp), delete=False)
...
def main(screen):
    ...
    if pac_cli_path is None:
        # test pac-cli in path -- TODO make robust
        try:
            run_command('pac-cli getinfo')
        except subprocess.CalledProcessError:
            quit(
                "--> cannot find pac-cli in $PATH\n" +
                "    do: export PATH=/path/to/pac-cli-folder:$PATH\n" +
                "    and try again\n")

    loadwin = curses.newwin(40, 40, 1, 2)

    loadwin.addstr(1, 2, 'pacvote version: ' + version, C_CYAN)

    mncount = int(run_pac_cli_command('masternode count enabled'))
    block_height = int(run_pac_cli_command('getblockcount'))
    ...
    # get ballot
    ballots = json.loads(run_pac_cli_command('gobject list all'))
    ballot = {}
    ...
    for entry in ballots:
        ...
        ballots[entry][u'votes'] = json.loads(run_pac_cli_command('gobject getvotes %s' % entry))
    ...
    masternodes = {}
    with open(os.path.join(pac_conf_dir, 'masternode.conf'), 'r') as f:
        lines = list(
            line
            for line in
            (l.strip() for l in f)
        ...
        for line in lines:
            ...
            masternodes[conf[3] + '-' + conf[4]] = {
                ...
                "txout": conf[4]}
    if not masternodes:
        # fallback to pac.conf entries if no masternode.conf entries
        with open(os.path.join(pac_conf_dir, 'pac.conf'), 'r') as f:
            lines = list(
                line
                for line in
                (l.strip() for l in f)
            ...
            if all(k in conf for k in ('masternode', 'externalip', 'masternodeprivkey')):
                # get funding tx from pacninja
                import urllib2
                mninfo = urllib2.urlopen(
                    "https://pacninja.pl/api/masternodes?ips=[\"" +
                ...
                try:
                    ...
                    quit('cannot retrieve masternode info from pacninja')
                ...
                quit('cannot find masternode information in pac.conf')
    ...
    votewin.addstr(1, 2, 'pacvote version: ' + version, C_CYAN)
    votewin.addstr(
        2,
        2,
